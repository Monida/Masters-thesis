...
Ylg2=
%}
function CMC = cmcError(RSref,RS) 
 F=100; %number of frames
 P=2; %number of recording systmes
 numOfTrial=size(RSref,2);
 %number of waveforms/Gait cycles
 
 for t=1:numOfTrial
    if size(RSref{t},2)<size(RS{t},2)
        numOfGC(t)=size(RSref{t},2);
    else
        numOfGC(t)=size(RS{t},2);
    end
end
 ...
 for t=1:numOfTrial
     Y1(p:(numOfGC(t)+(p-1)))=RSref{t}(1:numOfGC(t));
     Y2(p:(numOfGC(t)+(p-1)))=RS{t}(1:numOfGC(t));
     p=p+numOfGC(t);
 end
